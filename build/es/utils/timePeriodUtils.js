import { parse, format, addMonths, addWeeks, subMonths, subWeeks, startOfISOWeek, endOfISOWeek, startOfMonth, endOfMonth, getISOWeek, isAfter, isSameMonth, isSameYear, isValid, getISOWeekYear } from 'date-fns';
...
const getMonths = (start, end) => {
  try {
    ...
    while (isAfter(endDate, currentDate) || isSameYear(currentDate, endDate) && isSameMonth(currentDate, endDate)) {
 ...
 */
export const convertServerToClientPeriod = (periodId, periodType) => {
  try {
 ...
 */
export const comparePeriods = (a, b, periodType) => {
  if (periodType.toUpperCase() === PERIOD_TYPES.MONTH) {
 ...
 */
export const getLastNPeriods = (basePeriod, periodType, count) => {
  try {
    if (count <= 0) {
