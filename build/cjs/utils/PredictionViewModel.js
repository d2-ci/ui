...
const createPredictionPoint = period => ({
  period,
  periodLabel: (0, _multiCalendarDates.createFixedPeriodFromPeriodId)({
    periodId: period,
    calendar: 'gregory'
  }).displayName,
  quantiles: {}
});
function buildPredictionSeries(predictionEntries, orgUnitsById, targetId, actualCases) {
  ...
  return Array.from(byOrgUnit.entries()).map(([orgUnitId, periodsMap]) => {
    ...
    return {
      ...
      actualCases: actualCases === null || actualCases === void 0 ? void 0 : actualCases.filter(ac => ac.ou === orgUnitId && ac.value !== null).map(ac => ({
        period: ac.pe,
        value: ac.value
